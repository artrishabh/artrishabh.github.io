# Rishabh Jain Portfolio

Static portfolio for GitHub Pages, built with HTML, CSS, and JavaScript.

## Included in this version

- Updated Experience section with Scopely and OVERDARE details
- Two embedded Technical Art showreels
- Expandable area for future Technical Art articles and breakdowns
- Internal storefront-style pages for 17 games and prototypes
- Searchable and filterable full game catalogue
- itch.io download/play links, gameplay videos, screenshots, controls, credits, and project metadata
- GitHub, itch.io, and LinkedIn icon links
- Updated email and social contact information

## Publish on GitHub Pages

1. Extract the ZIP.
2. Open the extracted `artrishabh.github.io` folder.
3. Upload the files and folders inside it to the root of your GitHub repository.
4. Replace existing files when prompted.
5. Commit the upload to `main`.

The site is configured for:

- Repository: `artrishabh/artrishabh.github.io`
- Website: `https://artrishabh.github.io/`

## Local preview

Run this command inside the folder:

```bash
python -m http.server 8000
```

Then open `http://localhost:8000`.
